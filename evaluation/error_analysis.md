...
|---|---|---|---|---|
| 1 | Hebrew handwritten **explanation transcription** — two modes (controlled diagnostic, evaluation/diag_q1_2.md): **2a omission** (null under batch load; what the gate sees) and **2b confabulation** (single-item/cropped calls fabricate fluent Hebrew at ANY resolution incl. a 2600 px cell crop, and a cell-only crop even fabricated the answer letter) | 36 of 91 correct selections zeroed on null transcriptions = **144 points** across audited runs (lower bound) | dominant: up to −64 pts/exam | Open (model capability — proven not to be geometry/masking/resolution/parsing/caching). Gated zeros now review-flagged. **Per-row transcription crops WITHDRAWN as a fix** (they convert safe omissions into unsafe confabulations); the honest paths are the 32B bake-off with a transcription-fidelity gate, or instructor policy on provisional selection credit |
