...
| Exam setup | 1 course -> 2 exam (package or upload) -> 3 key / rubric / solution -> 4 detect structure (automatic: variants, alignment) -> 5 resolve package issues (policies, preflight; once per exam, never per student) -> 6 ready (students' scans, create batch) |
...
Owner tool (not part of the lecturer UI): `python -m streamlit run scripts/grade_label_ui.py`
creates the human grading labels for the GRADE_PRIMARY benchmark.
