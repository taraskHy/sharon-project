...
@live_db
@pytest.mark.skipif(not os.environ.get("LABELING_ALLOW_LIVE_DB"),
                    reason="opt-in only (LABELING_ALLOW_LIVE_DB=1): the default suite never opens "
                           "the deployment database, not even read-only")
def test_the_live_ground_truth_survives_every_evidence_version(tmp_path):
    ...
    label did not depend on what the app showed.

    The live database is NEVER opened here. `LabelDB.__init__` is a WRITER — it
    sets journal_mode and runs DDL/migrations — so pointing it at the live file
    while the labeling server holds it open risks corrupting the one copy of the
    ground truth. The check runs on a read-only online-backup snapshot instead.
    """
    from labeling_app.backup import snapshot_sqlite

    snap = tmp_path / "labels.db"
    try:
        snapshot_sqlite(LIVE_DB, snap)             # read-only connection, no write to the source
        with sqlite3.connect(f"file:{snap}?mode=ro", uri=True) as probe:
            health = probe.execute("PRAGMA integrity_check").fetchone()[0]
    except sqlite3.DatabaseError as e:
        pytest.skip(f"the live labeling database is not readable ({e}); this test reports on the "
                    "owner's deployment, not on this code — restore it from a backup first")
    if health != "ok":
        pytest.skip(f"the live labeling database fails integrity_check ({health.splitlines()[0]}); "
                    "restore it from %LOCALAPPDATA%\\autograder\\labeling\\backups before relying on it")
    db = LabelDB(snap)
    rep = db.verify_provenance()
...
def test_no_test_ever_opens_the_live_labeling_database_for_writing():
    """A guard against the mistake this file once made. `LabelDB.__init__` sets
    journal_mode and runs DDL — it is a WRITER. Constructing it on the live
    database while the labeling server holds it open can corrupt the only copy
    of the ground truth. Tests must snapshot first."""
    import re
    tests_dir = Path(__file__).resolve().parent
    offenders = []
    for path in sorted(tests_dir.glob("test_*.py")):
        src = path.read_text(encoding="utf-8")
        if "AppData" not in src and "LOCALAPPDATA" not in src:
            continue
        for n, line in enumerate(src.splitlines(), 1):
            if re.search(r"LabelDB\(\s*LIVE_DB", line) or re.search(r"LabelDB\([^)]*AppData", line):
                offenders.append(f"{path.name}:{n}: {line.strip()}")
    assert offenders == [], (
        "these tests would open the LIVE labeling database read-write; snapshot it "
        "with labeling_app.backup.snapshot_sqlite and open the copy instead:\n  " + "\n  ".join(offenders))
